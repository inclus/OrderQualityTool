    ...
    def run(self, cycle):
        ...
        for formulation in formulation_values:
            ...
            for facility_record in FacilityCycleRecord.objects.filter(cycle=cycle):
                current_values = FacilityConsumptionRecord.objects.filter(facility_cycle=facility_record, formulation=name).order_by().values('opening_balance', 'art_consumption', 'estimated_number_of_new_patients')
                new_values = FacilityConsumptionRecord.objects.filter(facility_cycle__facility=facility_record.facility, facility_cycle__cycle=next_cycle, formulation=name).order_by().values('opening_balance', 'art_consumption', 'estimated_number_of_new_patients')
