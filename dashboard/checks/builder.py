...
def build_model(name, check_type):
    ...
    if check_type and check_type["id"] == FACILITY_TWO_GROUPS_WITH_SAMPLE:
        ...
        model["tracingFormulations"] = [
            tracing_formulation.as_dict_obj()
            for tracing_formulation in TracingFormulations.objects.all()
        ]
    return model
...
class DefinitionFactory(object):
    ...
    class Builder(object):
        ...
        def nnrti_percentage_variance_is_less_than(self, ratio=1):
            self.data[OPERATOR] = {
                "id": "NNRTILessThan",
                NAME: "FOR NNRTI Percentage Variance is less than N%",
            }
            self.data["operatorConstant"] = ratio
            return self
    ...
    def initial(self):
        ...
        return self.Builder(data).type(FACILITY_TWO_GROUPS).aggregation(
            "SUM"
        ).formulations(
            "form_a", "form_b"
        ).fields(
    ...
    def traced(self, **kwargs):
        return self.initial().type(FACILITY_TWO_GROUPS_WITH_SAMPLE).sample(**kwargs)

    def based_on_class(self, class_name):
        data = {"type": "ClassBased", "pythonClass": class_name}
        return self.Builder({}).type(
            "ClassBased", "Class Based Facility Check"
        ).python_class(
            class_name
        )
...
def guideline_paed1l_check():
    ...
    builder.add_group(
        ...
        PAED_MODEL,
        ["new"],
        ["AZT/3TC/ABC", "AZT/3TC/EFV", "AZT/3TC/LPV/r", "AZT/3TC/NVP"],
    )
    builder.at_least_of_total(80)
...
def open_closing_check():
    builder = DefinitionFactory().blank().type(FACILITY_TWO_GROUPS_WITH_SAMPLE)
    builder.add_group(
        "G1", VALUE, CURRENT_CYCLE, CONSUMPTION_MODEL, ["opening_balance"], []
    )
    builder.add_group(
        "G2", VALUE, PREVIOUS_CYCLE, CONSUMPTION_MODEL, ["closing_balance"], []
    )
    builder.are_equal()
    return builder.get()
...
def warehouse_fulfillment_check():
    builder = DefinitionFactory().blank().type(FACILITY_TWO_GROUPS_WITH_SAMPLE)
    builder.add_group(
        "G1", VALUE, CURRENT_CYCLE, CONSUMPTION_MODEL, ["quantity_received"], []
    )
    builder.add_group(
        "G2", VALUE, PREVIOUS_CYCLE, CONSUMPTION_MODEL, ["packs_ordered"], []
    )
    builder.are_equal()
    return builder.get()
...
def nnrti_paed():
    ...
    builder.add_group(
        ...
        factors={
            "Efavirenz (EFV) 200mg [Pack 90]": "0.5",
...
def nnrti_adult():
    ...
    builder.add_group(
        ...
        factors={
            "Atazanavir/Ritonavir (ATV/r) 300mg/100mg [Pack 30]": "0.5",
