...
class UserDefinedCheckTestCase(TestCase):
    ...
    def test_check(self):
        has_no_negatives = LocationData.migrate_from_dict(
            {
                C_RECORDS: [{OPENING_BALANCE: 3, FORMULATION: F1_QUERY}],
                "status": "reporting",
            }
        )
    ...
    def test__group_values_from_location_data(self):
        ...
        output = check.get_values_from_records(
            test_location_data.c_records,
            group.selected_formulations,
            group.selected_fields,
        )
        self.assertEqual(
            output, [DataRecord.from_list([F1_QUERY, 3], group.selected_fields)]
        )
...
insufficient_data = LocationData.migrate_from_dict(
    {
        ...
        C_RECORDS: [{FORMULATION: F1_QUERY, COMBINED_CONSUMPTION: 40}],
    }
)
        ...
        C_RECORDS: [
            {
                ...
                ESTIMATED_NUMBER_OF_NEW_ART_PATIENTS: 40,
                ESTIMATED_NUMBER_OF_NEW_PREGNANT_WOMEN: 20,
            },
            {
                FORMULATION: "AZT/3TC 300/150mg",
                ESTIMATED_NUMBER_OF_NEW_ART_PATIENTS: 40,
                ESTIMATED_NUMBER_OF_NEW_PREGNANT_WOMEN: 20,
            },
            {
                FORMULATION: "AZT/3TC/NVP 300/150/200mg",
                ESTIMATED_NUMBER_OF_NEW_ART_PATIENTS: 40,
                ESTIMATED_NUMBER_OF_NEW_PREGNANT_WOMEN: 20,
            },
        ],
        ...
        C_RECORDS: [
            {
                FORMULATION: "Tenofovir/Lamivudine/Efavirenz (TDF/3TC/EFV) 300mg/300mg/600mg[Pack 30]",
                ESTIMATED_NUMBER_OF_NEW_ART_PATIENTS: 40,
                ESTIMATED_NUMBER_OF_NEW_PREGNANT_WOMEN: 50,
            },
            {
                FORMULATION: "AZT/3TC 300/150mg",
...
class TestGuideLineAdherence(TestCase):
    ...
    def test_check_can_handle(self, name, data, expected_result):
        ...
        legacy_check_result = legacy_check.for_each_facility(
            data, legacy_check.combinations[0]
        )
...
has_no_negatives = LocationData.migrate_from_dict(
    {"status": "reporting", C_RECORDS: [{OPENING_BALANCE: 3, FORMULATION: F1_QUERY}]}
)

has_blanks = LocationData.migrate_from_dict(
    {"status": "reporting", C_RECORDS: [{OPENING_BALANCE: None, FORMULATION: F1_QUERY}]}
)
