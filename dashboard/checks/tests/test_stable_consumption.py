...
from dashboard.data.entities import (
    LocationData,
    C_RECORDS,
    FORMULATION,
    F1_QUERY,
...
f1_current_consumption_zero = {
    "Current": LocationData.migrate_from_dict(
        {
            "status": "reporting",
            C_RECORDS: [{FORMULATION: F1_QUERY, COMBINED_CONSUMPTION: 0}],
        }
    ),
    "Previous": LocationData.migrate_from_dict(
        {
...
f1_consumption_falls_by_50 = {
    "Current": LocationData.migrate_from_dict(
        {
            "status": "reporting",
            C_RECORDS: [{FORMULATION: F1_QUERY, COMBINED_CONSUMPTION: 30}],
        }
    ),
    "Previous": LocationData.migrate_from_dict(
        {
            "status": "reporting",
            C_RECORDS: [{FORMULATION: F1_QUERY, COMBINED_CONSUMPTION: 60}],
        }
    ),
}
...
class StableConsumptionCheckTestCase(TestCase):
    ...
    def test_that_legacy_gets_same_result_as_new(self, name, scenario, expected):
        ...
        new_check_result = new_check.for_each_facility(
            scenario["Current"], Tracer.F1(), scenario["Previous"]
        )
    ...
    def test_that_check_has_same_result_as_preview(
        self, name, scenario, combination, expected
    ):
        ...
        data = new_check.get_preview_data(
            {"name": location.facility, "district": location.district},
            current_cycle,
            combination,
        )
...
class StablePatients(TestCase):
    ...
    def test_check_runs(self):
        ...
        fake_import = FakeImport(
            locations, consumption_records, adult_records, paed_records
        )
        run_facility_test(
