...
class VolumeTallyCheck(QCheck):
    ...
    def for_each_facility(self, data, tracer, previous_cycle_data=None):
        ...
        df2_records = get_patient_records(
            data, tracer.patient_formulations, tracer.extras[IS_ADULT]
        )
        ...
        if (
            ...
            and (
                ...
                or (
                    divisible
                    and calculate_percentage_variance(
                        implied_patient_volume, actual_patient_volumes
                    )
                    < 30
                )
            )
