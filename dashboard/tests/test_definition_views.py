...
from dashboard.tests.test_helpers import (
    gen_adult_record,
    gen_paed_record,
    gen_consumption_record,
)
    ...
    def test_that_serializer_can_handle_test_of_type_class_based(self):
        serializer = DefinitionSerializer(
            data={
                "python_class": "test",
                "groups": [],
                "type": {"id": "ClassBased", "name": "ClassBased"},
            }
        )
...
class PreviewViewTestCase(WebTest):
    ...
    def test_that_a_sample_is_required(self):
        ...
        assert_that(
            json_response,
            has_entry(
                equal_to("sample"), has_item(equal_to("This field is required."))
            ),
        )

    def test_that_the_factors_are_considered(self):
        gen_paed_record()
        params = DefinitionFactory().sampled().are_equal().model("Paed").factors(
            form_b=2, form_a=5
        ).get()
        response = self.app.post_json(
            self.url, user="testuser", params=params, expect_errors=True
        )
        json_response = loads(response.content.decode("utf8"))
        self.assertEqual(200, response.status_code)
        assert_that(
    ...
    def test_two_facility_with_tracing_type(self):
        gen_consumption_record(
            formulation="form_tra", opening_balance=10, closing_balance=20
        )
        params = DefinitionFactory().traced(
            tracer={"slug": "trace1", "formulations": ["form_tra", "form_trb"]}
        ).model(
            "Consumption"
        ).tracing_formulations(
            "consumption_formulations", "form_tra", "form_trb"
        ).fields(
            "opening_balance", "closing_balance"
        ).are_equal().get()
        response = self.app.post_json(
    ...
    def test_equal_to(self):
        gen_consumption_record(
            formulation="form1", opening_balance=10, closing_balance=20
        )
        gen_consumption_record(
            formulation="form2", opening_balance=40, closing_balance=30
        )
                    ...
                    has_entries(
                        {
                            ...
                            "aggregation": "SUM",
                            "values": equal_to(
                                [["form1", 10.0, 20.0], ["form2", 40.0, 30.0]]
                            ),
                            "headers": equal_to(["opening_balance", "closing_balance"]),
        ...
        assert_that(
            json_response,
            has_entry(equal_to("result"), has_entries({"DEFAULT": equal_to("YES")})),
        )
    ...
    def test_that_models_can_be_overridden(self):
        ...
        definition_builder.model("Paed").model_overrides(
            {"trace1": {"id": "Adult", "formulations": ["form_tra"]}}
        )
        definition_builder.tracing_formulations(
            "patient_formulations", "form_tra", "form_trb"
        )
        definition_builder.are_equal()
        definition = definition_builder.get()
        response = self.app.post_json(
            self.url, user="testuser", params=definition, expect_errors=True
        )
        json_response = loads(response.content.decode("utf8"))
        self.assertEqual(200, response.status_code)
        assert_that(
