...
from dashboard.checks.legacy.adherence import (
    GuidelineAdherenceCheckAdult1L,
    calculate_score,
)
from dashboard.checks.legacy.blanks import (
    BlanksQualityCheck,
    IsReportingCheck,
    MultipleCheck,
)
...
class DataTestCase(TestCase):
    ...
    def test_consumption_records(self):
        data = LocationData.migrate_from_dict(
            {
                C_RECORDS: [
                    {FORMULATION: "A", OPENING_BALANCE: 3},
    ...
    def test_full_html_import(self):
        fixture = get_test_output_from_fixture(
            "arv-0-consumption-data-report-maul.html", report_type=CONSUMPTION_REPORT
        )
        fixture.extend(
        ...
        self.assertEqual(
            Score.objects.filter(cycle=test_cycle)[0].data["WAREHOUSE FULFILMENT"],
            {
...
class GuidelineAdherenceAdult1LTestCase(TestCase):
    ...
    def test_score_is_yes_if_sum_of_new_hiv_positive_women_and_new_art_for_tdf_is_80_percent_that_for_AZT(
        self
    ):
    ...
    def test_score_is_no_if_tdf_cells_are_blank(self):
        ...
        result = calculate_score(
            df1_count, df2_count, sum_df1, sum_df2, ratio, True, False
        )
    ...
    def test_score_is_no_if_azt_cells_are_blank(self):
        ...
        ratio = 0.8
        result = calculate_score(
            df1_count, df2_count, sum_df1, sum_df2, ratio, False, True
        )
        self.assertEqual(result, NO)
