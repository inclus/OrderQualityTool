...
from dashboard.data.html_data_import import (
    extract_locations_and_import_records,
    HtmlDataImport,
)
...
def get_test_output_from_fixture(name, report_type=PAED_PATIENT_REPORT, warehouse=MAUL):
    ...
    report = Dhis2StandardReport.objects.filter(
        report_type=report_type, warehouse=warehouse
    ).first()
...
class PaedImportCombinedTestCase(TestCase):
    ...
    def setUp(self):
        ...
        results = get_test_output_from_fixture(
            "arv-2-paediatric-art-patient-report-maul.html"
        )
        results.extend(
            get_test_output_from_fixture(
        ...
        self.data_import = HtmlDataImport(results, None).load(
            LocationToPartnerMapping.get_mapping()
        )
...
class ConsumptionImportTestCase(TestCase):
    ...
    def test_that_test_location_records_have_correct_values(self):
        ...
        self.assertEqual(first_record.consumption, 194)
...
class ConsumptionCombinedImportTestCase(TestCase):
    ...
    def test_that_test_location_records_have_correct_values(self):
        ...
        self.assertEqual(first_record.consumption, 388)
        self.assertEqual(first_record.loses_adjustments, 0)
